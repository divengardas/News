...
class NewsApp:
    ...
    def __init__(self):
        ...
        self.data = requests.get(
            'https://newsapi.org/v2/top-headlines?country=us&apiKey=ffb20caf7b4846e6bdfc95a7c7af02ba').json()
        # initial GUI load
        self.load_gui()
        # load 1st news item
    ...
    def load_news_item(self, index):
        ...
        label = Label(self.root, image=photo, relief=RAISED)
        label.pack()

        heading = Label(self.root, text=self.data['articles'][index]['title'],
                        bg='black', fg='white', wraplength=350, justify='center', relief=RAISED)
        heading.pack(pady=(10, 20))
        heading.config(font=('verdana', 15))

        details = Label(self.root, text=self.data['articles'][index]['description'],
                        bg='white', fg='black', wraplength=350, justify='center', relief=RAISED)
        details.pack(pady=(2, 2))
